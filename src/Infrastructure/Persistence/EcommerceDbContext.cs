...
public class EcommerceDbContext : IdentityDbContext<Usuario>
{
    ...
    {
        builder.Entity<Category>().HasMany(p => p.Products).WithOne(r => r.Category)
        .HasForeignKey(r => r.CategoryId)
        .IsRequired().OnDelete(DeleteBehavior.Restrict);

        builder.Entity<Product>().HasMany(p => p.Reviews)
        ...
        builder.Entity<ShoppinCart>().HasMany(p => p.ShoppingCartItems)
        .WithOne(r => r.ShoppinCart).HasForeignKey(r => r.ShoppingCartId)
        .IsRequired().OnDelete(DeleteBehavior.Cascade);
